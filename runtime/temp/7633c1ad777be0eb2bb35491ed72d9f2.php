<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:60:"/www/wwwroot/crm.yuanhang.org/template/business/sku_put.html";i:1527125566;s:66:"/www/wwwroot/crm.yuanhang.org/template/business/public_header.html";i:1526259101;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
	<meta charset="UTF-8">
...
<body>
  ...
  <div class="main">
    <div class="notice"> 您当前的位置：<a href="<?php echo url('common/index'); ?>">首页</a> &gt; <a href="<?php echo url('sku/index'); ?>?sku=put" class="v-link-active">入库</a> </div>
    <div class="main_content">
      ...
      <form class="layui-form">
        ...
        <div class="layui-inline mr15">
          <input class="layui-input" name="order_num" id="demoReload" autocomplete="off" placeholder="采购单号" value="<?php echo $order_num; ?>">
        </div>
        ...
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="in_time" class="layui-input" id="test16" placeholder="开始 到 结束" value="<?php echo $in_time; ?>">
          </div>
        </div>
	  ...
	  <?php if((in_array('Sku/put_add',$user['account_auth'])) and ($user['position'] != 1)): ?>
      <a class="btn mr10" href="javascript:;" id="add-put">添加</a>
	  <?php elseif($user['position'] == 1): ?>
	  <a class="btn mr10" href="javascript:;" id="add-put">添加</a>
	  <?php endif; if((in_array('Sku/put_del',$user['account_auth'])) and ($user['position'] != 1)): ?>
	  <a class="btn do-action" href="javascript:;" data-type="doDelete" data-href="<?php echo url('sku/put_del'); ?>">删除</a>	
	  <?php elseif($user['position'] == 1): ?>
	  <a class="btn do-action" href="javascript:;" data-type="doDelete" data-href="<?php echo url('sku/put_del'); ?>">删除</a>	
	  <?php endif; ?>
      <div class="table">
        <table class="layuitable">
          ...
          <tbody>
            <?php if(is_array($put_list) || $put_list instanceof \think\Collection || $put_list instanceof \think\Paginator): $i = 0; $__LIST__ = $put_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            <tr>
              ...
              <td><?php echo date("Y-m-d",$vo['in_time']); ?></td>
              <td><?php echo date("Y-m-d",$vo['check_time']); ?></td>
              <td><?php echo $vo['status']; ?></td>
              <td><?php echo $vo['put_operator']; ?></td>
              <td>
				<?php if((in_array('Sku/put_del',$user['account_auth'])) and ($user['position'] != 1)): ?>
				<a class="blue do-action" id="del" href="javascript:;" data-type="doDelOne" data-href="<?php echo url('sku/put_del'); ?>" data-id="<?php echo $vo['id']; ?>">删除</a>
				<?php elseif($user['position'] == 1): ?>
				<a class="blue do-action" id="del" href="javascript:;" data-type="doDelOne" data-href="<?php echo url('sku/put_del'); ?>" data-id="<?php echo $vo['id']; ?>">删除</a>
				<?php endif; ?>
			  </td>
            ...
            layer.close(layer.index);
	            if(data.code==1){
	                layer.alert(data.msg, {icon: 6}, function(index){
	                    layer.close(index);
	                    window.location.href = "<?php echo url('sku/put'); ?>";
	                });
	            }else{
...
function gopage(page){
  ...
  window.location.href="<?php echo url('sku/put'); ?>?plat_name=<?php echo $plat_id; ?>&order_num=<?php echo $order_num; ?>&card_id=<?php echo $card_id; ?>&in_time=<?php echo $in_time; ?>&page="+page;
}
